...
import sys
import time                          #Time imported for correct UTC seconds
...
def encode_string(string):
        ...
        for c in string.upper():
                ...
                encoded_string += '{0:b}'.format(index).rjust(6,'0')  #6 as six bit ascii
...
def encode_14(__mmsi, __msg):
        ...
        _mmsi = '{0:b}'.format(__mmsi).rjust(30,'0')		# 30 bits (247320162)
        _spare = "00"										# spare bit
        _msg = encode_string(__msg)                             #Consider (__msg + 'X') if final char is truncated?
...
def encode_21(__mmsi, __aid_type, __aid_name, __long, __lat, __vsize, __virtual):
		...
		# I'VE COMMNETED OUT THE ABOVE AND FIXED THE ATON SIZE AS 2m BY 2m BELOW                
                _half_length = '000000001'
                _half_width = '000001'
        else:
        ...
        UTC_Sec = int(strftime("%S", gmtime()))

        _time = '{0:b}'.format(UTC_Sec).rjust(6,'0')
        _virtual = '{0:b}'.format(int(__virtual))

        #Line bwlow commneted out as it gives an off position indication
...
def main():
        ...
        #Prints binary AIS message payload
        print (payload)
        
        NMEAgenerator(payload)
