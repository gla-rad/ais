...
Build_Frame::sptr Build_Frame::make(const char* sentence, bool repeat, bool enable_NRZI)
{
    return gnuradio::make_block_sptr<Build_Frame_impl>(sentence, repeat, enable_NRZI);
}
...
void Build_Frame_impl::create_payload(const char* sentence)
{
    ...
    if (REMAINDER_TO_EIGHT == 0) {
        ...
        // %d bits to %d\n", d_len_payload, PADDING_TO_EIGHT, d_len_payload +
        // PADDING_TO_EIGHT);
                           ...
                           gr_vector_void_star& output_items)
{
    ...
    if (d_len_payload <= 168) {
        ...
        int LEN_STUFFED_PAYLOAD =
            stuff(d_payload, stuffed_payload, d_len_payload + LEN_CRC);
