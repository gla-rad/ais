...
namespace gr {
  namespace aistx {
    ...
    const int udp_source_vb_impl::BUF_SIZE_PAYLOADS = 
        gr::prefs::singleton()->get_long("udp_blocks", "buf_size_payloads", 50);

    udp_source_vb::sptr
    udp_source_vb::make(size_t itemsize, const std::string& host, int port, int payload_size, bool eof)
    {
      return gnuradio::make_block_sptr<udp_source_vb_impl>(
     ...
     */
    udp_source_vb_impl::udp_source_vb_impl(size_t itemsize, const std::string& host, int port, int payload_size, bool eof)
      : gr::sync_block("udp_source_vb",
              ...
              d_sent(0)
    {
        GR_LOG_WARN(d_logger, itemsize);
        GR_LOG_WARN(d_logger, d_payload_size);
        GR_LOG_WARN(d_logger, BUF_SIZE_PAYLOADS);
     ...
     */
    udp_source_vb_impl::~udp_source_vb_impl()
    {
        if (d_connected)
            disconnect();
    ...
    void udp_source_vb_impl::connect(const std::string& host, int port)
    {
        if (d_connected)
            disconnect();
        ...
        if (!host.empty()) {
            ...
            start_receive();
            d_udp_thread =
                gr::thread::thread(boost::bind(&udp_source_vb_impl::run_io_service, this));
            d_connected = true;
        }
    }

    void udp_source_vb_impl::disconnect()
    {
    ...
    int udp_source_vb_impl::get_port(void)
    {
        // return d_endpoint.port();
        return d_socket->local_endpoint().port();
    }

    void udp_source_vb_impl::start_receive()
    {
        d_socket->async_receive_from(
            boost::asio::buffer((void*)d_rxbuf, d_payload_size),
            d_endpoint_rcvd,
            boost::bind(&udp_source_vb_impl::handle_read,
                        this,
                        boost::asio::placeholders::error,
                        boost::asio::placeholders::bytes_transferred));
    }

    void udp_source_vb_impl::handle_read(const boost::system::error_code& error,
    ...
    int
    udp_source_vb_impl::work(int noutput_items,
